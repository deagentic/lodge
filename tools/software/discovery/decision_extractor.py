...
MAGIC_NUMBER_RE = re.compile(r"\b(0x[0-9A-Fa-f]{2,8}|[0-9]{3,})\b")
...
RETRY_RE = re.compile(
    r"(?:retry|retries|attempts|max_attempts|max_retry)\s*[=:]\s*(\d+)", re.IGNORECASE
)

BYTE_SEQUENCE_RE = re.compile(r"(?:0x[0-9A-Fa-f]{2}\s*[,\s]+){2,}")
...
def extract_decisions(path: Path, tech_stack: dict) -> dict:
    ...
    critical = [
        _to_dict(d)
        for d in all_decisions
        if d.kind in ("protocol", "constant")
        or (
            d.kind == "comment"
            and d.description.startswith(("HACK", "WORKAROUND", "BUG", "FIXME"))
        )
    ]
...
def _process_constant_line(
    line: str, stripped: str, rel: str, i: int, decisions: list[Decision]
) -> None:
    ...
    if APDU_COMMAND_RE.search(line) and (
        "0xFF" in line or "0x00" in line or "FF" in line.upper()
    ):
        decisions.append(
            Decision(
                kind="protocol",
...
def _process_byte_array_line(
    line: str, i: int, array_bytes: list, array_start: int, rel: str, decisions: list
) -> bool:
        ...
        if len(array_bytes) >= 4:
            decisions.append(
                Decision(
                    ...
                    file=rel,
                    line=array_start,
                    snippet=f"Array starting line {array_start}",
                )
            )
        return False
    return True
...
def _scan_byte_sequences(lines: list[str], rel: str) -> list[Decision]:
    ...
    for i, line in enumerate(lines, 1):
        if re.search(r"(?:byte|uint8|BYTE|char)\s*\w*\s*\[", line):
            ...
            array_bytes = []
        if in_array:
            in_array = _process_byte_array_line(
                line, i, array_bytes, array_start, rel, decisions
            )
...
def _process_winscard_line(
    line: str, rel: str, i: int, decisions: list[Decision]
) -> None:
