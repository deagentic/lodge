...
def _collect_public_api(modules: list) -> list[dict]:
    ...
    for mod in modules:
        for cls in mod.get("classes", []):
            public_api.append(
                {"name": cls["name"], "kind": "class", "location": mod["path"]}
            )
        for fn in mod.get("functions", []):
            if not fn["name"].startswith("_"):
                public_api.append(
                    {"name": fn["name"], "kind": "function", "location": mod["path"]}
                )
    return public_api
...
def _parse_python(f: Path, rel: str) -> Optional[dict]:
    ...
    is_entry = (
        ...
        or f.name in ("main.py", "app.py", "run.py", "cli.py")
    )
...
def _parse_c(f: Path, rel: str) -> Optional[dict]:
    ...
    classes = [
        {"name": m, "line": _line_of(source, m), "methods": []}
...
def _parse_js(f: Path, rel: str) -> Optional[dict]:
    ...
    classes = [
        {"name": m[0], "bases": [m[1]] if m[1] else [], "methods": []}
    ...
    return {
        ...
        in ("index.js", "index.ts", "main.js", "main.ts", "app.js", "app.ts"),
        "line_count": source.count("\n"),
    }
...
_CS_CLASS_RE = re.compile(
    r"(?:public|private|internal|protected|static|sealed|abstract)\s+(?:partial\s+)?class\s+(\w+)"
)
...
def _parse_csharp(f: Path, rel: str) -> Optional[dict]:
    ...
    return {
        ...
        "classes": classes,
        "functions": functions,
        "is_entry_point": "static void Main" in source
        or "static async Task Main" in source,
        "line_count": source.count("\n"),
    }
