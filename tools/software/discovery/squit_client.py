...
class SquitClient:
    ...
    def dependencies(self, object_name, limit=20):
        return self._call_tool(
            "squit_dependencies", {"object_name": object_name, "limit": limit}
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--action", required=True, choices=["search", "get_code", "deps"]
    )
    ...
    if not args.key:
        print(
            "[ERROR] SQUIT_API_KEY not set. Add it to your .env file or pass --key.",
            file=sys.stderr,
        )
        sys.exit(1)
